...
bitclone_binary="${bitclone_binary-"${TEST_SRCDIR}/${TEST_WORKSPACE}/java/com/khulnasoft/bitclone/bitclone"}"

function bitclone() {
  $bitclone_binary --jvm_flag=-Djava.util.logging.config.file=$log_config "$@" \
      --output-root "$repo_storage" \
      --work-dir "$workdir" > $TEST_log 2>&1 \
      && return

  res=$?
  printf 'Bitclone process returned error %d:\n' $res
  cat $TEST_log
  return $res
}
...
# Runs Bitclone and check that the exit code is the expected one.
function bitclone_with_exit_code() {
  local expected_code=$1
  shift
  bitclone "$@" && status=$? || status=$?
...
function check_bitclone_rev_id() {
   local repo="$1"
   local primary=$(get_primary_branch $repo)
   check_bitclone_rev_id_branch "$repo" "$2" "$primary"
}

function check_bitclone_rev_id_branch() {
   local repo="$1"
   local origin_id="$2"
   local branch="$3"
   ( cd $repo || return
...
core.workflow(
    ...
    authoring = authoring.pass_thru("Bitclone Team <no-reply@khulnasoft.com>"),
    transformations = [
      core.replace(
        before = "food",
        after  = "drink"
...
EOF
  ...
  check_bitclone_rev_id "$destination" "$first_commit"
  ...
  check_bitclone_rev_id "$destination" "$second_commit"
...
EOF
  ...
  bitclone bit.clone.sky default $commit_three --last-rev $commit_one

  check_bitclone_rev_id "$destination" "$commit_three"
  ...
  bitclone bit.clone.sky default $commit_five

  check_bitclone_rev_id "$destination" "$commit_five"
  ...
  bitclone_with_exit_code $NO_OP bit.clone.sky default $commit_three
...
EOF
  ...
  check_bitclone_rev_id_branch "$destination" "$commit_three" push
  ...
  check_bitclone_rev_id_branch "$destination" "$commit_five" push
...
EOF
  ...
  bitclone info bit.clone.sky default
  ...
  bitclone bit.clone.sky default $commit_one --force

  check_bitclone_rev_id "$destination" "$commit_one"
  ...
  bitclone bit.clone.sky default $commit_four
  ...
  check_bitclone_rev_id "$destination" "$commit_four"
  ...
  bitclone bit.clone.sky default $commit_five --last-rev $commit_three
...
EOF
  ...
  bitclone bit.clone.sky default $commit_three --last-rev $commit_primary --force
...
EOF
  ...
  bitclone bit.clone.sky export $commit_two
  ...
  bitclone bit.clone.sky import_change $pr_request
...
EOF
  bitclone bit.clone.sky forward --force
  ...
  bitclone bit.clone.sky reverse --force
...
function test_local_dir_destination() {
  ...
  cat > destination/bit.clone.sky <<EOF
core.workflow(
    name = "default",
    origin = git.origin(
      url = "file://$remote",
    ...
    destination_files = glob(include = ["**"], exclude = ["bit.clone.sky", "**.keep"]),
...
EOF
  ...
  bitclone destination/bit.clone.sky --folder-dir destination

  [[ -f destination/test.txt ]] || fail "test.txt should exist"
  [[ -f destination/bit.clone.sky ]] || fail "bit.clone.sky should exist"
...
EOF
  ...
  bitclone destination/bit.clone.sky choochoochoose_me --folder-dir destination
...
EOF
  ...
  bitclone destination/bit.clone.sky --folder-dir destination
  expect_log "ioRepoTask.*PROFILE:.*[0-9]* //bitclone/clean_outputdir"
  expect_log "repoTask.*PROFILE:.*[0-9]* //bitclone/run/default/origin.resolve_source_ref"
  expect_log "doMigrate.*PROFILE:.*[0-9]* //bitclone/run/default/squash/prepare_workdir"
  expect_log "checkout.*PROFILE:.*[0-9]* //bitclone/run/default/squash/origin.checkout"
  expect_log "transform.*PROFILE:.*[0-9]* //bitclone/run/default/squash/transforms/Moving test.txt"
  expect_log "transform.*PROFILE:.*[0-9]* //bitclone/run/default/squash/transforms/Moving test.moved"
  expect_log "doMigrate.*PROFILE:.*[0-9]* //bitclone/run/default/squash/transforms"
  expect_log "doMigrate.*PROFILE:.*[0-9]* //bitclone/run/default/squash/destination.write"
  expect_log "run.*PROFILE:.*[0-9]* //bitclone/run/default/squash"
  expect_log "run.*PROFILE:.*[0-9]* //bitclone/run"
  expect_log "shutdown.*PROFILE:.*[0-9]* //bitclone"
...
function test_command_args_logged() {
  bitclone foo bar baz --option && fail "should fail"
  expect_log 'Running: .*foo bar baz --option'
}

function test_command_bitclone_filename_no_correct_name() {
  bitclone_with_exit_code $CONFIGURATION_ERROR migrate somename.bzl
  expect_log "Bitclone config file filename should be 'bit.clone.sky'"
}
...
function test_reversible_check() {
  ...
  bitclone_with_exit_code $CONFIGURATION_ERROR bit.clone.sky --force
  expect_log "ERROR: Workflow 'default' is not reversible"
}

function test_disable_reversible_check() {
  setup_reversible_check_workflow
  bitclone --disable-reversible-check bit.clone.sky --force
}
...
EOF
  bitclone_with_exit_code $SUCCESS INFO bit.clone.sky --info-list-only
  expect_log "another_workflow,one_workflow"
}

function test_config_not_found() {
  bitclone_with_exit_code $COMMAND_LINE_ERROR bit.clone.sky origin/primary
  expect_log "Configuration file not found: bit.clone.sky"
}

#Verify that we instantiate LogConsole when System.console() is null
function test_no_ansi_console() {
  bitclone_with_exit_code $COMMAND_LINE_ERROR bit.clone.sky
  expect_log "^[0-9]\{4\} [0-2][0-9]:[0-5][0-9]:[0-5][0-9].*"
}

# Verify that Bitclone fails if we try to read the input from the user from a writeOnly LogConsole
...
EOF
  bitclone_with_exit_code $INTERNAL_ERROR bit.clone.sky --force
...
EOF
  cat > $config_folder/foo/bar/bit.clone.sky <<EOF
...
EOF
  ...
  bitclone foo/bar/bit.clone.sky $flags --force
...
function test_subcommand_parsing_fails() {
  bitclone_with_exit_code $COMMAND_LINE_ERROR migrate.sky bit.clone.sky

  expect_log "Invalid subcommand 'migrate.sky'"
}

function test_command_bitclone_wrong_subcommand() {
  bitclone_with_exit_code COMMAND_LINE_ERROR foooo
  expect_log "Invalid subcommand 'foooo'. Available commands: "
  expect_log "Try 'bitclone help'"
}

function test_default_command_too_few_args() {
  bitclone_with_exit_code $COMMAND_LINE_ERROR
  expect_log "Configuration file missing for 'migrate' subcommand."
  expect_log "Try 'bitclone help'"
}

function test_migrate_missing_config() {
  bitclone_with_exit_code $COMMAND_LINE_ERROR migrate
...
EOF
  ...
  expect_log "Configuration '.*bit.clone.sky' is valid."
}
...
EOF
  ...
  bitclone_with_exit_code $CONFIGURATION_ERROR validate bit.clone.sky

  expect_log "Configuration '.*bit.clone.sky' is invalid."
...
EOF
  ...
  bitclone_with_exit_code $CONFIGURATION_ERROR migrate bit.clone.sky
...
EOF
  ...
  bitclone bit.clone.sky default $commit_primary --last-rev $commit_initial
  bitclone bit.clone.sky default $commit_one --last-rev $commit_primary
...
EOF
  ...
  bitclone_with_exit_code $CONFIGURATION_ERROR bit.clone.sky default --last-rev --some-other-flag

  expect_log "Invalid refspec: --some-other-flag"
}

run_suite "Integration tests for Bitclone code sharing tool."
