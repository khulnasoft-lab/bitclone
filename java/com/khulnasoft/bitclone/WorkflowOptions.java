...
import com.khulnasoft.bitclone.jcommander.AuthorConverter;
import com.khulnasoft.bitclone.jcommander.GlobConverter;
...
public class WorkflowOptions implements Option {
  ...
  @Parameter(
      ...
      description =
          "Commit revision to be used as parent when importing a commit using CHANGE_REQUEST"
              + " workflow mode. This shouldn't be needed in general as Bitclone is able to detect"
  ...
  @Parameter(
      ...
      description =
          "Re-import the last version imported. This is useful for example to check that"
              + " a refactor in a bit.clone.sky file doesn't introduce accidental changes.")
  ...
  @Parameter(names = {"--import-noop-changes"},
      description = "By default Bitclone will only try to migrate changes that could affect the"
  ...
  @Parameter(names = CHECK_LAST_REV_STATE,
      description = "If enabled, Bitclone will validate that the destination didn't change"
  ...
  @Parameter(names = "--notransformation-join",
      description = "By default Bitclone tries to join certain transformations in one so that it"
  ...
  @Parameter(names = "--diff-in-origin",
      description = "When this flag is enabled, bitclone will show different changes between last"
  ...
  @Parameter(
      ...
      description =
          "Reverse apply the existing patch files in the destination to obtain a baseline for merge"
              ...
              + " This patch handling process correctly represents changes introduced by bitclone"
