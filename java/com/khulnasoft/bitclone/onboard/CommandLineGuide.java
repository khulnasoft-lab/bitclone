...
/** Class to navigate users through text adventure to populate Bitclone config file */
final class CommandLineGuide {
  ...
  public static void runForCommandLine(CommandEnv commandEnv) {
    ...
    console.info("Welcome to Bitclone's Assisted Onboarding Tool!\n");
    com.khulnasoft.bitclone.onboard.ConfigBuilder configBuilder =
        new com.khulnasoft.bitclone.onboard.ConfigBuilder(
            new com.khulnasoft.bitclone.onboard.GitToGitTemplate());
    Set<com.khulnasoft.bitclone.onboard.RequiredField> requiredFields =
        configBuilder.getRequiredFields();
    for (com.khulnasoft.bitclone.onboard.RequiredField field : requiredFields) {
      String response =
          tryAskConsole(
              console,
              String.format(
