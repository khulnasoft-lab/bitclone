...
import static com.khulnasoft.bitclone.Workflow.BITCLONE_CONFIG_PATH_IDENTITY_VAR;
import static com.khulnasoft.bitclone.config.GlobalMigrations.getGlobalMigrations;
...
import static com.khulnasoft.bitclone.config.SkylarkUtil.convertOptionalString;
import static com.khulnasoft.bitclone.config.SkylarkUtil.convertStringList;
import static com.khulnasoft.bitclone.config.SkylarkUtil.convertStringMap;
...
import static com.khulnasoft.bitclone.transform.Transformations.toTransformation;
...
import com.khulnasoft.bitclone.exception.EmptyChangeException;
...
import com.khulnasoft.bitclone.templatetoken.Parser;
import com.khulnasoft.bitclone.templatetoken.Token;
import com.khulnasoft.bitclone.templatetoken.Token.TokenType;
import com.khulnasoft.bitclone.transform.CopyOrMove;
import com.khulnasoft.bitclone.transform.ExplicitReversal;
import com.khulnasoft.bitclone.transform.FilterReplace;
import com.khulnasoft.bitclone.transform.Remove;
import com.khulnasoft.bitclone.transform.Rename;
import com.khulnasoft.bitclone.transform.Replace;
import com.khulnasoft.bitclone.transform.ReplaceMapper;
import com.khulnasoft.bitclone.transform.ReversibleFunction;
import com.khulnasoft.bitclone.transform.Sequence;
import com.khulnasoft.bitclone.transform.SkylarkConsole;
import com.khulnasoft.bitclone.transform.SkylarkTransformation;
import com.khulnasoft.bitclone.transform.TodoReplace;
import com.khulnasoft.bitclone.transform.TodoReplace.Mode;
import com.khulnasoft.bitclone.transform.VerifyMatch;
...
public class Core implements LabelsAwareModule, StarlarkValue {
  ...
  @StarlarkMethod(
      ...
      doc =
          "Defines a migration pipeline which can be invoked via the Bitclone command.\n"
              ...
              + "\n"
              + "  - "
              + TransformWork.BITCLONE_CONTEXT_REFERENCE_LABEL
              + ": Requested reference. For example if bitclone is invoked as `bitclone"
              + " bit.clone.sky workflow master`, the value would be `master`.\n"
              + "  - "
              + TransformWork.BITCLONE_LAST_REV
              + ": Last reference that was migrated\n"
              + "  - "
              + TransformWork.BITCLONE_CURRENT_REV
              + ": The current reference being migrated\n"
              + "  - "
              + TransformWork.BITCLONE_CURRENT_REV_DATE_TIME
              + ": Date & time for the current reference being migrated in ISO format"
              + " (Example: \"2011-12-03T10:15:30+01:00\")\n"
              + "  - "
              + TransformWork.BITCLONE_CURRENT_MESSAGE
              + ": The current message at this point of the transformations\n"
              + "  - "
              + TransformWork.BITCLONE_CURRENT_MESSAGE_TITLE
              + ": The current message title (first line) at this point of the transformations\n"
              + "  - "
              + TransformWork.BITCLONE_AUTHOR
      ...
      parameters = {
        ...
        @Param(
            ...
            named = true,
            doc =
                "If set to true, Bitclone will validate that the destination didn't change"
        ...
        @Param(
            ...
            doc =
                "By default, Bitclone hashes several fields so that each change has an unique"
                    ...
                    + " identity is used in several workflows. At least ${bitclone_config_path}"
                    + " has to be present. Current user is added to the hash"
                    + " automatically.<br><br>Available variables:<ul> "
                    + " <li>${bitclone_config_path}: Main config file path</li> "
                    + " <li>${bitclone_workflow_name}: The name of the workflow being run</li> "
                    + " <li>${bitclone_reference}: The requested reference. In general Bitclone"
        ...
        @Param(
            ...
            named = true,
            doc =
                "Bitclone adds labels like 'GitOrigin-RevId' in the destination in order to"
        ...
        @Param(
            ...
            named = true,
            doc =
                "Perform these transformations after merge_import, but before Bitclone writes to"
        ...
        @Param(
            ...
            named = true,
            doc =
                "By default, Bitclone tries to only migrate changes that affect origin_files or"
  ...
  private static ImmutableList<Token> getChangeIdentity(Object changeIdentityObj)
    ...
    for (Token token : result) {
      ...
      if (token.getValue().equals(BITCLONE_CONFIG_PATH_IDENTITY_VAR)) {
        configVarFound = true;
        continue;
      }
      if (token.getValue().equals(Workflow.BITCLONE_WORKFLOW_NAME_IDENTITY_VAR)
          || token.getValue().equals(Workflow.BITCLONE_REFERENCE_IDENTITY_VAR)
          || token.getValue().startsWith(Workflow.BITCLONE_REFERENCE_LABEL_VAR)) {
        continue;
      }
      throw Starlark.errorf("Unrecognized variable: %s", token.getValue());
    }
    check(configVarFound, "${%s} variable is required", BITCLONE_CONFIG_PATH_IDENTITY_VAR);
    return result;
  }
  ...
  @StarlarkMethod(
      name = "feedback",
      doc =
          "Defines a migration of changes' metadata, that can be invoked via the Bitclone command"
