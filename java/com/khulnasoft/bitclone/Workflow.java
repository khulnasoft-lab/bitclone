...
import static com.khulnasoft.bitclone.LazyResourceLoader.memoized;
import static com.khulnasoft.bitclone.WorkflowMode.CHANGE_REQUEST;
import static com.khulnasoft.bitclone.WorkflowMode.CHANGE_REQUEST_FROM_SOT;
...
import com.khulnasoft.bitclone.feedback.FinishHookContext;
...
import com.khulnasoft.bitclone.util.Identity;
...
public class Workflow<O extends Revision, D extends Revision> implements Migration {
  ...
  static final String BITCLONE_CONFIG_PATH_IDENTITY_VAR = "bitclone_config_path";
  static final String BITCLONE_WORKFLOW_NAME_IDENTITY_VAR = "bitclone_workflow_name";
  static final String BITCLONE_REFERENCE_IDENTITY_VAR = "bitclone_reference";
  static final String BITCLONE_REFERENCE_LABEL_VAR = "label:";
  ...
  public void run(Path workdir, ImmutableList<String> sourceRefs)
    ...
    try (ProfilerTask ignore = profiler().start("run/" + name)) {
      ...
      logger.atInfo().log(
              "Running Bitclone for workflow '%s' and ref '%s': %s",
   ...
   * Bitclone invocations for the same reference. For example it will contain the bit.clone.sky
  ...
  String getMigrationIdentity(Revision requestedRevision, TransformWork transformWork) {
    ...
    for (Token token : changeIdentity) {
      ...
      } else if (token.getValue().equals(BITCLONE_CONFIG_PATH_IDENTITY_VAR)) {
        sb.append(mainConfigFile.getIdentifier());
      } else if (token.getValue().equals(BITCLONE_WORKFLOW_NAME_IDENTITY_VAR)) {
        sb.append(this.name);
      } else if (token.getValue().equals(BITCLONE_REFERENCE_IDENTITY_VAR)) {
        sb.append(ctxRef);
      } else if (token.getValue().startsWith(BITCLONE_REFERENCE_LABEL_VAR)) {
        String label = token.getValue().substring(BITCLONE_REFERENCE_LABEL_VAR.length());
