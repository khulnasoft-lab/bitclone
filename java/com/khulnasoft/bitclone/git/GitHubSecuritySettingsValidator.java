  ...
  private boolean hasAllStar(String organization) throws ValidationException, RepoException {
    try {
      ...
      throw handleGitHubException(
          ...
          "Please review your bitclone app permissions, this request requires admin:read"
              + " permissions.");
    }
  }
  ...
  private boolean hasTwoFactorEnabled(String organization, Console console)
    ...
    try {
      ...
      if (twoFactorEnabled == null) {
        console.warnFmt(
            "Bitclone could not confirm that 2FA requirement is being enforced in the '%s' GitHub"
                + " organization, so it will be assumed as being not enforced."
                + " Please confirm Bitclone is given admin:org permissions with your GitHub org"
