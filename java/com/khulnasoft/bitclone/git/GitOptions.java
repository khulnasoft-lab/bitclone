...
import static com.khulnasoft.bitclone.util.FileUtil.resolveDirInCache;
...
import com.khulnasoft.bitclone.jcommander.GreaterThanZeroValidator;
...
public class GitOptions implements Option {
  ...
  @DynamicParameter(
      ...
      description =
          "This is a repeatable flag used to set command level configurations, currently only"
              + " applies to git merge operations. E.g. bitclone bit.clone.sky --git-cmd-config"
              + " google.foo=bar --git-cmd-config google.baz=qux would make git operations done by"
              + " bitclone under the hood use the -c flags: git -c google.foo=bar -c google.baz=qux"
              + " ...")
  ...
  @Parameter(
      ...
      description =
          "This is a repeatable flag used to set git push level flags to send to git servers. E.g."
              + " bitclone bit.clone.sky --git-push-option foo --git-push-option bar would make git"
              + " operations done by bitclone under the hood use the --push-option flags: git push"
  ...
  @Parameter(
      ...
      description =
          "This is a flag used to allowlist push options sent to git servers. E.g. bitclone"
              + " bit.clone.sky --git-push-option=\"foo,bar\" would make bitclone validate push so"
  ...
  @Parameter(names = "--git-tag-overwrite",
      description = "If set, bitclone will force update existing git tag")
  boolean gitTagOverwrite = false;

  @Parameter(names = "--experiment-checkout-affected-files",
      description = "If set, bitclone will only checkout affected files at git origin. "
