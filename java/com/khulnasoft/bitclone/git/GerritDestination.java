...
import com.khulnasoft.bitclone.git.gerritapi.ChangeInfo;
import com.khulnasoft.bitclone.git.gerritapi.ChangesQuery;
import com.khulnasoft.bitclone.git.gerritapi.GerritApi;
import com.khulnasoft.bitclone.git.gerritapi.IncludeResult;
import com.khulnasoft.bitclone.git.gerritapi.SetReviewInput;
import com.khulnasoft.bitclone.git.gerritapi.SubmitInput;
...
public final class GerritDestination implements Destination<GitRevision> {
  ...
  static class GerritWriteHook implements WriteHook {
     ...
     * not because bitclone prints a different output based on if the change already exists in
     * Gerrit or not.
     */
    @Override
    ...
    private String computeInternalHashTag(TransformResult result) {
      return "bitclone_id_" + result.getChangeIdentity()
    ...
    private Optional<ChangeInfo> findActiveChange(String hashTag)
      ...
      if (changes.size() > 1) {
        console.warnFmt("Multiple changes found for the same internal bitclone tag: %s. Reusing"
                + " %s",
