...
public class CommandEnvTest {
  ...
  public void testConfigParsingNoWorkflow() throws CommandLineException {
    CommandEnv env = new CommandEnv(workdir, options, ImmutableList.of("foo/bit.clone.sky"));
    env.parseConfigFileArgs(mock(BitcloneCmd.class), /*usesSourceRef=*/true);
    assertThat(env.getConfigFileArgs().getConfigPath()).isEqualTo("foo/bit.clone.sky");
  ...
  public void testConfigParsingNoSourceRef() throws CommandLineException {
    CommandEnv env = new CommandEnv(workdir, options, ImmutableList.of("foo/bit.clone.sky", "wf"));
    env.parseConfigFileArgs(mock(BitcloneCmd.class), /*usesSourceRef=*/true);
    assertThat(env.getConfigFileArgs().getConfigPath()).isEqualTo("foo/bit.clone.sky");
  ...
  public void testConfigParsingNoSourceRefButPassed() {
    CommandEnv env = new CommandEnv(workdir, options,
        ImmutableList.of("foo/bit.clone.sky", "wf", "foo"));
    CommandLineException e =
        assertThrows(
            CommandLineException.class,
            () -> env.parseConfigFileArgs(mock(BitcloneCmd.class), /*usesSourceRef=*/ false));
  ...
  public void testConfigParsing() throws CommandLineException {
    CommandEnv env = new CommandEnv(workdir, options,
        ImmutableList.of("foo/bit.clone.sky", "wf", "123"));
    env.parseConfigFileArgs(mock(BitcloneCmd.class), /*usesSourceRef=*/true);
    assertThat(env.getConfigFileArgs().getConfigPath()).isEqualTo("foo/bit.clone.sky");
    assertThat(env.getConfigFileArgs().getWorkflowName()).isEqualTo("wf");
  ...
  public void testConfigParsingMultipleSourceRefs() throws CommandLineException {
    CommandEnv env = new CommandEnv(workdir, options,
        ImmutableList.of("foo/bit.clone.sky", "wf", "123", "456"));
    env.parseConfigFileArgs(mock(BitcloneCmd.class), /*usesSourceRef=*/true);
    assertThat(env.getConfigFileArgs().getConfigPath()).isEqualTo("foo/bit.clone.sky");
    assertThat(env.getConfigFileArgs().getWorkflowName()).isEqualTo("wf");
