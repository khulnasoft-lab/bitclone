...
import com.khulnasoft.bitclone.Core;
...
public class FilterReplaceTest {
  ...
  public void testGroupOptional() throws Exception {
    ...
    write("file1.txt", ""
        + "#import <foo>\n"
        + "#import <aaaa>bitclone\n"
        + "");
    write("file2.txt", "#import<other>\n");

    transform(filterReplace(""
        + "regex = '#import <(.*)>(bitclone)?',"
        + "mapping = {"
        + "    'bitclone': 'baracopy',\n"
        + "},"
        + "group = 2,"
  ...
  public void testCustomReverse() throws Exception {
    ...
    Transformation t = filterReplace(""
        + "regex = '#import <fo.*>',"
        + "reverse = '#import <.*> // by Bitclone',"
        + "mapping = {"
        + "    '#import <foo1>': '#import <bar1> // by Bitclone',\n"
        + "    '#import <foo2>': '#import <bar2> // by Bitclone',\n"
        + "},"
    );
    transform(t);
    ...
    assertThatPath(checkoutDir)
        .containsFile("file.txt",""
            + "#import <bar1> // by Bitclone\n"
            + "#import <bar2> // by Bitclone\n"
            + "#import <bar1>\n")
        .containsNoMoreFiles();
