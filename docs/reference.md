...
reverse | `string` or `NoneType`<br><p>The reverse of the command. This is only required if the given command cannot be reversed automatically and the reversal of this command is required by some workflow or Bitclone check. The following commands are automatically reversible:<br><ul><li>add</li><li>remove (when used to remove element from list i.e. 'remove srcs foo.cc'</li><li>replace</li></ul></p>
...
migrated | `sequence of `[`change`](#change)<br><p>List of changes that where migrated in previous Bitclone executions or if using ITERATIVE mode in previous iterations of this workflow.</p>
...
Show an error in the log. Note that this will stop Bitclone execution.
...
---- | ---- | -----------
<span style="white-space: nowrap;">`--allow-empty-diff`</span> | *boolean* | If set to false, Bitclone will not write to the destination if the exact same change is already pending in the destination. Currently only supported for `git.github_pr_destination` and `git.gerrit_destination`.
...
<span style="white-space: nowrap;">`--console-file-flush-interval`</span> | *duration* | How often Bitclone should flush the console to the output file. (10s, 1m, etc.)If set to 0s, console will be flushed only at the end.  Example values: 30s, 20m, 1h, etc.
...
<span style="white-space: nowrap;">`--force`</span> | *boolean* | Force the migration even if Bitclone cannot find in the destination a change that is an ancestor of the one(s) being migrated. This should be used with care, as it could lose changes when migrating a previous/conflicting change.
...
<span style="white-space: nowrap;">`--output-root`</span> | *string* | The root directory where to generate output files. If not set, ~/bitclone/out is used by default. Use with care, Bitclone might remove files inside this root if necessary.
...
<span style="white-space: nowrap;">`--version-selector-use-cli-ref`</span> | *boolean* | If command line ref is to used with a version selector, pass this flag to tell bitclone to use it.
...
regex_groups | `dict`<br><p>A set of named regexes that can be used to match part of the file name. Bitclone uses [re2](https://github.com/khulnasoft/re2/wiki/Syntax) syntax. For example {"x": "[A-Za-z]+"}</p>
...
Defines a migration of changes' metadata, that can be invoked via the Bitclone command in the same way as a regular workflow migrates the change itself.
...
regex_groups | `dict`<br><p>A set of named regexes that can be used to match part of the replaced text.Bitclone uses [re2](https://github.com/khulnasoft/re2/wiki/Syntax) syntax. For example {"x": "[A-Za-z]+"}</p>
paths | [`glob`](#glob) or `NoneType`<br><p>A glob expression relative to the workdir representing the files to apply the transformation. For example, glob(["**.java"]), matches all java files recursively. Defaults to match all the files recursively.</p>
...
Replace KhulnaSoft style TODOs. For example `TODO(username, othername)`.
...
Defines a migration pipeline which can be invoked via the Bitclone command.
...
Implicit labels that can be used/exposed:
  ...
  - BITCLONE_CURRENT_MESSAGE: The current message at this point of the transformations
  - BITCLONE_CURRENT_MESSAGE_TITLE: The current message title (first line) at this point of the transformations
  - BITCLONE_AUTHOR: The author of the change
...
check_last_rev_state | `bool`<br><p>If set to true, Bitclone will validate that the destination didn't change since last-rev import for destination_files. Note that this flag doesn't work for CHANGE_REQUEST mode.</p>
...
change_identity | `string` or `NoneType`<br><p>By default, Bitclone hashes several fields so that each change has an unique identifier that at the same time reuses the generated destination change. This allows to customize the identity hash generation so that the same identity is used in several workflows. At least ${bitclone_config_path} has to be present. Current user is added to the hash automatically.<br><br>Available variables:<ul>  <li>${bitclone_config_path}: Main config file path</li>  <li>${bitclone_workflow_name}: The name of the workflow being run</li>  <li>${bitclone_reference}: The requested reference. In general Bitclone tries its best to give a repetable reference. For example Gerrit change number or change-id or GitHub Pull Request number. If it cannot find a context reference it uses the resolved revision.</li>  <li>${label:label_name}: A label present for the current change. Exposed in the message or not.</li></ul>If any of the labels cannot be found it defaults to the default identity (The effect would be no reuse of destination change between workflows)</p>
set_rev_id | `bool`<br><p>Bitclone adds labels like 'GitOrigin-RevId' in the destination in order to track what was the latest change imported. For `CHANGE_REQUEST` workflows it is not used and is purely informational. This field allows to disable it for that mode. Destinations might ignore the flag.</p>
...
after_merge_transformations | `sequence`<br><p>Perform these transformations after merge_import, but before Bitclone writes to the destination. Ex: any BUILD file generations that rely on the results of merge_import</p>
migrate_noop_changes | `bool`<br><p>By default, Bitclone tries to only migrate changes that affect origin_files or config files. This flag allows to include all the changes. Note that it might generate more empty changes errors. In `ITERATIVE` mode it might fail if some transformation is validating the message (Like has to contain 'PUBLIC' and the change doesn't contain it because it is internal).</p>
...
<span style="white-space: nowrap;">`--change-request-parent, --change_request_parent`</span> | *string* | Commit revision to be used as parent when importing a commit using CHANGE_REQUEST workflow mode. This shouldn't be needed in general as Bitclone is able to detect the parent commit message.
<span style="white-space: nowrap;">`--check-last-rev-state`</span> | *boolean* | If enabled, Bitclone will validate that the destination didn't change since last-rev import for destination_files. Note that this flag doesn't work for CHANGE_REQUEST mode.
...
<span style="white-space: nowrap;">`--diff-in-origin`</span> | *boolean* | When this flag is enabled, bitclone will show different changes between last Revision and current revision in origin instead of in destination. NOTE: it Only works for SQUASH and ITERATIVE
...
<span style="white-space: nowrap;">`--import-noop-changes`</span> | *boolean* | By default Bitclone will only try to migrate changes that could affect the destination. Ignoring changes that only affect excluded files in origin_files. This flag disables that behavior and runs for all the changes.
<span style="white-space: nowrap;">`--init-history`</span> | *boolean* | Import all the changes from the beginning of the history up to the resolved ref. For 'ITERATIVE' workflows this will import individual changes since the first one. For 'SQUASH' it will import the squashed change up to the resolved ref. WARNING: Use with care, this flag should be used only for the very first run of Bitclone for a workflow.
...
Would copy all files in path/to/ from the destination baseline to the bitclone workdir. The files do not have to be covered by origin_files nor destination_files, but will cause errors if they are not covered by destination_files and not moved or deleted.
...
Name | Type | Description
---- | ---- | -----------
<span style="white-space: nowrap;">`--folder-dir`</span> | *string* | Local directory to write the output of the migration to. If the directory exists, all files will be deleted. By default Bitclone will generate a temporary directory, so you shouldn't need this.
...
Module for formatting the code to KhulnaSoft's style/guidelines
...
--------- | -----------
id | `string`<br><p>The change id or change number.</p>
include_results | `sequence of string`<br><p>What to include in the response. See https://gerrit-review.khulnasoft.com/Documentation/rest-api-changes.html#query-options</p>

<a id="gerrit_api_obj.list_changes" aria-hidden="true"></a>
### gerrit_api_obj.list_changes

Get changes from Gerrit based on a query. See https://gerrit-review.khulnasoft.com/Documentation/rest-api-changes.html#list-changes.
...
Parameter | Description
--------- | -----------
query | `string`<br><p>The query string to list changes by. See https://gerrit-review.khulnasoft.com/Documentation/user-search.html#_basic_change_search.</p>
include_results | `sequence of string`<br><p>What to include in the response. See https://gerrit-review.khulnasoft.com/Documentation/rest-api-changes.html#query-options</p>
...
Input for listing Gerrit changes. See https://gerrit-review.khulnasoft.com/Documentation/rest-api-changes.html#list-changes
...
Creates a commit in a git repository using the transformed worktree.<br><br>For GitHub use git.github_destination. For creating Pull Requests in GitHub, use git.github_pr_destination. For creating a Gerrit change use git.gerrit_destination.<br><br>Given that Bitclone doesn't ask for user/password in the console when doing the push to remote repos, you have to use ssh protocol, have the credentials cached or use a credential manager.
...
integrates | `sequence of git_integrate` or `NoneType`<br><p>Integrate changes from a url present in the migrated change label. Defaults to a semi-fake merge if BITCLONE_INTEGRATE_REVIEW label is present in the message</p>
primary_branch_migration | `bool`<br><p>When enabled, bitclone will ignore the 'push' and 'fetch' params if either is 'master' or 'main' and instead try to establish the default git branch. If this fails, it will fall back to the param's declared value.<br>This is intended to help migrating to the new standard of using 'main' without breaking users relying on the legacy default.</p>
...
Creates a change in Gerrit using the transformed worktree. If this is used in iterative mode, then each commit pushed in a single Bitclone invocation will have the correct commit parent. The reviews generated can then be easily done in the correct order without rebasing.
...
partial_fetch | `bool`<br><p>This is an experimental feature that only works for certain origin globs.</p>
notify | `string` or `NoneType`<br><p>Type of Gerrit notify option (https://gerrit-review.khulnasoft.com/Documentation/user-upload.html#notify). Sends notifications by default.</p>
...
allow_empty_diff_patchset | `bool`<br><p>By default Bitclone will upload a new PatchSet to Gerrit without checking the previous one. If this set to false, Bitclone will download current PatchSet and check the diff against the new diff.</p>
...
gerrit_submit | `bool`<br><p>By default, Bitclone uses git commit/push to the main branch when submit = True.  If this flag is enabled, it will update the Gerrit change with the latest commit and submit using Gerrit.</p>
primary_branch_migration | `bool`<br><p>When enabled, bitclone will ignore the 'push_to_refs_for' and 'fetch' params if either is 'master' or 'main' and instead try to establish the default git branch. If this fails, it will fall back to the param's declared value.<br>This is intended to help migrating to the new standard of using 'main' without breaking users relying on the legacy default.</p>
...
Implicit labels that can be used/exposed:
  ...
  - BITCLONE_INTEGRATE_REVIEW: A label that when exposed, can be used to integrate automatically in the reverse workflow.
...
api_checker | `checker` or `NoneType`<br><p>A checker for the Gerrit API endpoint provided for after_migration hooks. This field is not required if the workflow hooks don't use the origin/destination endpoints.</p>
primary_branch_migration | `bool`<br><p>When enabled, bitclone will ignore the 'push' and 'fetch' params if either is 'master' or 'main' and instead try to establish the default git branch. If this fails, it will fall back to the param's declared value.<br>This is intended to help migrating to the new standard of using 'main' without breaking users relying on the legacy default.</p>
tag_name | `string` or `NoneType`<br><p>A template string that specifies to a tag name. If the tag already exists, bitclone will only overwrite it if the --git-tag-overwrite flag is set.<br>Note that tag creation is best-effort and the migration will succeed even if the tag cannot be created. Usage: Users can use a string or a string with a label. For instance ${label}_tag_name. And the value of label must be in changes' label list. Otherwise, tag won't be created.</p>
tag_msg | `string` or `NoneType`<br><p>A template string that refers to the commit msg for a tag. If set, bitclone willcreate an annotated tag with this custom message<br>Usage: Labels in the string will be resolved. E.g. .${label}_message.By default, the tag will be created with the labeled commit's message.</p>
checker | `checker` or `NoneType`<br><p>A checker that validates the commit files & message. If `api_checker` is not set, it will also be used for checking API calls. If only `api_checker`is used, that checker will only apply to API calls.</p>
...
Parameter | Description
--------- | -----------
url | `string`<br><p>Url of the GitHub project. For example "https://github.com/khulnasoft/bitclone'"</p>
...
update_description | `bool`<br><p>By default, Bitclone only set the title and body of the PR when creating the PR. If this field is set to true, it will update those fields for every update.</p>
primary_branch_migration | `bool`<br><p>When enabled, bitclone will ignore the 'desination_ref' param if it is 'master' or 'main' and instead try to establish the default git branch. If this fails, it will fall back to the param's declared value.<br>This is intended to help migrating to the new standard of using 'main' without breaking users relying on the legacy default.</p>
checker | `checker` or `NoneType`<br><p>A checker that validates the commit files & message. If `api_checker` is not set, it will also be used for checking API calls. If only `api_checker`is used, that checker will only apply to API calls.</p>
...
Create a branch by using bitclone's computerIdentity algorithm:

```python
git.github_pr_destination(
        url = "https://github.com/khulnasoft/bitclone",
...
`git_integrate` `git.integrate(label="BITCLONE_INTEGRATE_REVIEW", strategy="FAKE_MERGE_AND_INCLUDE_FILES", ignore_errors=True)`
...
refspec_groups | `dict`<br><p>A set of named regexes that can be used to match part of the versions. Bitclone uses [re2](https://github.com/khulnasoft/re2/wiki/Syntax) syntax. Use the following nomenclature n0, n1, n2 for the version part (will use numeric sorting) or s0, s1, s2 (alphabetic sorting). Note that there can be mixed but the numbers cannot be repeated. In other words n0, s1, n2 is valid but not n0, s0, n1. n0 has more priority than n1. If there are fields where order is not important, use s(N+1) where N ist he latest sorted field. Example {"n0": "[0-9]+", "s1": "[a-z]+"}</p>
...
tag | `string` or `NoneType`<br><p>Tag to be applied to the review, for instance 'autogenerated:bitclone'.</p>
...
context | `string`<br><p>The context for the commit status. Use a value like 'bitclone/import_successful' or similar</p>
...
Get all the reference SHA-1s from GitHub. Note that Bitclone only returns a maximum number of 500.
...
A glob represents a set of relative filepaths in the Bitclone workdir.



## Globals

Global functions available in Bitclone
...
Bitclone uses Java globbing. The globbing is very similar to Bash one. This means that recursive globbing for a filename is a bit more tricky:
...
Parameter | Description
--------- | -----------
module | `string`<br><p>The go module path name. e.g. github.com/khulnasoft/gopacket. This will automatically normalize uppercase characters to '!{your_uppercase_character}' to escape them.</p>
...
Example of how create a version list for github.com/khulnasoft/gopacket

```python
go.go_proxy_version_list(
        module='github.com/khulnasoft/gopacket'
)
```
...
```python
metadata.add_header("BITCLONE CHANGE")
```
...
```python
metadata.add_header("BITCLONE CHANGE FOR https://github.com/myproject/foo/pull/${GITHUB_PR_NUMBER}",
...
```
BITCLONE CHANGE FOR https://github.com/myproject/foo/pull/1234
Example description for
documentation
```
...
```python
metadata.add_header("BITCLONE CHANGE: ", new_line = False)
```
...
all | `bool`<br><p>By default Bitclone tries to find the most relevant instance of the label. First looking into the message and then looking into the changes in order. If this field is true it exposes all the matches instead.</p>
concat_separator | `unknown`<br><p>If all is set, bitclone will expose multiple values in one per line. If a separator is specified, it will concat the values instead.</p>
...
metadata.map_author({
    ...
    'madeupexample@khulnasoft.com' : 'Other Person <someone@example.com>',
...
regex_groups | `dict`<br><p>Regexes for the ${reference} token's content. Requires one 'before_ref' entry matching the ${reference} token's content on the before side. Optionally accepts one 'after_ref' used for validation. Bitclone uses [re2](https://github.com/khulnasoft/re2/wiki/Syntax) syntax.</p>
additional_import_labels | `sequence of string`<br><p>Meant to be used when migrating from another tool: Per default, bitclone will only recognize the labels defined in the workflow's endpoints. The tool will use these additional labels to find labels created by other invocations and tools.</p>
...
```python
metadata.replace_message("BITCLONE CHANGE: Import of ${GITHUB_PR_NUMBER}\n\n${GITHUB_PR_BODY}\n")
```

Will transform the message to:

```
BITCLONE CHANGE: Import of 12345
...
search_all_changes | `bool`<br><p>By default Bitclone only looks in the last current change for the author label. This allows to do the search in all current changes (Only makes sense for SQUASH/CHANGE_REQUEST).</p>
...
msg_if_no_match | `string` or `NoneType`<br><p>If set, Bitclone will use this text when the scrubbing regex doesn't match.</p>
...
Set of functions to work with regexes in Bitclone.
...
Parameter | Description
--------- | -----------
project | `string`<br><p>The GitHub project from which to load the file, e.g. khulnasoft/bitclone</p>
...
`origin` `remotefiles.origin(author='Bitclone <noreply@bitclone.io>', message='Placeholder message', unpack_method='AS_IS', archive_source='', version_list=None, origin_version_selector=None, version_resolver=None)`
...
Input for posting a review to Gerrit. See https://gerrit-review.khulnasoft.com/Documentation/rest-api-changes.html#review-input
...
Module for parsing TOML in Bitclone.
...
whole_message | `bool`<br><p>By default Bitclone only looks in the last paragraph for labels. This flagmake it replace labels in the whole message.</p>
...
Set of functions to work with XML in Bitclone.
